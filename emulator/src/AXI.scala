...
class AXI4LoopBack(
  ...
  val io = IO(new Bundle {
    val axi = Flipped(AXI4Bundle(axiParams))
    val deq = Decoupled(UInt(axiParams.dataBits.W))
    val enq = Flipped(Decoupled(UInt(axiParams.dataBits.W)))
  })
  ...
  val serdes_deq = Module(new StreamWidthAdapter(axiParams.dataBits, axiParams.dataBits))
  serdes_deq.io.wide.in.bits     := 0.U
  serdes_deq.io.wide.in.valid    := false.B
  serdes_deq.io.narrow.out.ready := false.B

  val incomingQueueIO = Module(new Queue(UInt(axiParams.dataBits.W), 4)).io
  ...
  val serdes_enq = Module(new StreamWidthAdapter(axiParams.dataBits, axiParams.dataBits))
  // unused
  serdes_enq.io.narrow.in.bits  := 0.U
  serdes_enq.io.narrow.in.valid := false.B
  serdes_enq.io.wide.out.ready  := false.B

  val outgoingQueueIO = Module(new Queue(UInt(axiParams.dataBits.W), 4)).io
